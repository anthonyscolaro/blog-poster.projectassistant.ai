...
USING (
  organization_id IN (
    SELECT organization_id 
    FROM public.profiles 
    WHERE id = auth.uid() AND role IN ('owner', 'admin') AND deleted_at IS NULL
  )
);
...
-- Articles policies
CREATE POLICY "Organization members can view articles" 
ON public.articles 
FOR SELECT 
USING (
  organization_id IN (
...
CREATE POLICY "Organization members can create articles" 
ON public.articles 
FOR INSERT 
WITH CHECK (
  organization_id IN (
...
CREATE POLICY "Organization members can update articles" 
ON public.articles 
FOR UPDATE 
USING (
  organization_id IN (
...
CREATE POLICY "Organization members can create pipelines" 
ON public.pipelines 
FOR INSERT 
WITH CHECK (
  organization_id IN (
